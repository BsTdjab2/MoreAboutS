...
| Looks like | Actually |
| --- | --- |
| Contact form says "not connected yet" | Correct until the worker is deployed and `CONTACT_ENDPOINT` is set in `assets/js/config.js` |
| Firmware options greyed out | Correct until real URLs are added to `assets/data/firmware.json` |
| Three project cards say "Soon" | The owner's placeholder content, kept from the original |
