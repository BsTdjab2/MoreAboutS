...
export function initContactForm() {
  ...
  if (!CONTACT_ENDPOINT) {
    setStatus(status, 'info', 'The contact form is not connected yet. Use the Discord invite above to reach me.');
  }
  ...
  form.addEventListener('submit', async (event) => {
    ...
    if (!CONTACT_ENDPOINT) {
      setStatus(status, 'error', `Contact form not configured yet. Reach me on Discord: ${DISCORD_INVITE}`);
      return;
    }
    ...
    if (attachment && attachment.files[0]) payload.append('attachment', attachment.files[0]);
